...
export class TricksComponent implements OnInit {
    ...
    getMarkdown(): void {
        this.activatedRouter.params.subscribe(params => {
            const id = params.id;
            this.data =
                this.notionService.cheatsheet.find((cheatsheet: iCheatsheet) => cheatsheet.id === id) ||
                ({} as iCheatsheet);
            this.data.img_sanitized = this.sanitizer.bypassSecurityTrustResourceUrl(this.data.img) as string;
    ...
    addTargetBlank(html: string): string {
        return html.replace(new RegExp("<a", "g"), '<a class="link-trick" target="_blank" rel="nofollow" ');
    }
