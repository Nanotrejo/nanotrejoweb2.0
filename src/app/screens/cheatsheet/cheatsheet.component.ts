...
export class CheatsheetComponent implements OnInit {
    ...
    filter: Array<string> = ["Todo", "Angular", "CSS", "HTML", "JavaScript", "Python", "Linux"];
    ...
    async getCheatsheetData() {
        ...
        newData.sort((a: iCheatsheet, b: iCheatsheet) => new Date(b?.date)?.getTime() - new Date(a?.date)?.getTime());
        if (!this.storageService.checkObjectsAreEqual(this.cheatsheetData, newData, ["bento", "span", "img"])) {
            ...
            filter === "Todo"
                ...
                : this.cheatsheetDataAux.filter(
                      (cheatsheet: iCheatsheet) =>
                          ...
                          cheatsheet.description.toLowerCase().includes(filter.toLowerCase())
                  );
        this.filterBackdrop();
    }
    ...
    filterBackdrop() {
        ...
        const filterBackdrop = document.querySelector("#filter-backdrop") as HTMLElement;
        ...
        window.requestAnimationFrame(() => {
            ...
            listItem.forEach(item => {
                if (String(item.textContent).trim().toLowerCase() === this.filterSelected.toLowerCase()) {
