...
import { iAction, iSection, sectionData, TYPE_ACTION } from "@core/interface/action";
...
export class ActionService {
            ...
            section.action.forEach((action: iAction) => {
                if (action.typeAction === TYPE_ACTION.URL)
                    action.perform = () => {
                        ...
                        else this.router.navigate([action.url], { fragment: action.fragment });
                    };
    ...
    async setBlogData(): Promise<void> {
        this.blogData = [];
        let cheatsheetData: Array<iCheatsheet> = this.storageService.getItem(Storage.CHEATSHEET) || [];
        if (cheatsheetData.length === 0) cheatsheetData = await this.notionService.getCheatsheet();
